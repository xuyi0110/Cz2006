...
public class Loan extends Entity {
	...
	public ArrayList<Loan> getAllRequestLists() throws Exception {
		ArrayList<Loan> loans = new ArrayList<Loan>();
		DBconnect();
		ResultSet rs;
		...
		while (rs.next()) {
			Loan loan = new Loan(new Member(rs.getString("mId")), new Item(rs.getString("isbn")), Config.DATE_FORMAT.parse(rs.getString("date")));
			if (loan.checkRequestOverdue())
				loans.add(loan);
		}
	...
	public boolean checkRequestOverdue() throws Exception {
		if (brwDate == null) 
			getInfo();
		Date currentDate = new Date();
		int days = (int) ((currentDate.getTime() - brwDate.getTime()) / (24 * 60 * 60 * 1000));
		if (days > Config.RQUEST_DUE_DATE) {
			deleteFromRequst();
			return false;
		}
		return true;
